...
description: This skill should be used when coordinating complex work through a three-tier hierarchy of main manager, sub-manager, and worker roles with structured JSON communication, emergency escalation, and multi-agent platform support.
...
This skill defines a disciplined three-tier operating model for complex work that can overload a single agent's context. It keeps planning, decomposition, execution, validation, and escalation separated so the whole workflow stays stable as tasks grow.

The main manager remains responsible for user communication and the final merge. Sub-managers turn broad goals into micro-tasks, validate worker output, and keep each domain bounded. Workers execute only the assigned task and report back in structured form.

## Supported Surfaces

This skill is designed to work across multiple agent surfaces:
...
If a worker hits a critical failure such as server downtime, permission denial, data corruption, runaway recursion, infinite loop, or any other condition that prevents safe continuation, the worker must bypass the sub-manager and notify the main manager immediately.
...
OpenClaw support uses a workspace bootstrap file plus a short marker block in `SOUL.md`.

- Keep the bootstrap block small.
- Keep the hierarchy terse enough for repeated injection.
- Keep the role prompts in `agents/` so the workspace can route correctly.
...
This skill is intentionally strict. Predictable boundaries, clean handoffs, and immediate escalation are the point. If the situation is ambiguous, the safer choice is to stop, report, and ask for direction.
